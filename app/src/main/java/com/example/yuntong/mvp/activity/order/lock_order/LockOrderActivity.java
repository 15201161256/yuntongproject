 ...
 * @Describe 订单编辑锁定合同页面
 */
public class LockOrderActivity extends BaseActivity implements LockOrderContract.View {
    ...
    private LockOrderPresenter orderPresenter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        rowsBean = (OrderListbean.RowsBean) getIntent().getSerializableExtra("bean");
        id = rowsBean.getId() + "";
        orderNoEt.setText(rowsBean.getOrderNo());
        orderNameEt.setText(rowsBean.getOrderName());
        bigItemNoEt.setText(rowsBean.getBigItemNo());
        orderAmountEt.setText(rowsBean.getOrderAmount() + "");
        timeStartTv.setText(rowsBean.getStartTime());
        missNoEt.setText(rowsBean.getMissNo());
        taxRateEt.setText(rowsBean.getTaxRate());
        time = rowsBean.getStartTime();
    }
    ...
    protected void initView() {
        orderPresenter = new LockOrderPresenter(this, this);
        //获取当前年月日
    ...
    @OnClick({R.id.back_rl, R.id.ok_bt, R.id.edit_bt, R.id.start_time})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.back_rl:
                ...
                break;
            case R.id.ok_bt:
                orderPresenter.overOrder(ApiAddress.OVER_ORDER + ApiAddress.orderId);
                break;
            case R.id.edit_bt:
    ...
    private void Check(String contractId, String orderNo, String orderName,
        ...
        orderPresenter.editOrder(id, orderNo, orderName, bigItemNo, orderAmount, time,missNo, taxRate);
    }
    /**
     * 日期选择的回调监听
     */
    ...
    @Override
    public void getData(final BaseEntry<EditOrderBean> data) {
        if (data.getCode().equals("200")) {
            dialog = DialogUtil.showDialog2(mActivity, "订单锁定成功", new View.OnClickListener() {
                @Override
    ...
    @Override
    public void getEditData(final BaseEntry<EditOrderBean> data) {
        if (data.getCode().equals("200")) {
            dialog = DialogUtil.showDialog2(mActivity, "合同订单修改成功", new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    dialog.dismiss();
                    Intent intent = new Intent();
